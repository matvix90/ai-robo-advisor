...
def print_portfolio(portfolio: Portfolio) -> None:
    ...
    if portfolio.holdings:
        ...
        for h in portfolio.holdings:
            ...
            table.add_row(
                h.symbol, h.name, isin_display, h.asset_class, f"{h.weight:.2f}"
            )
        ...
        table.add_row(
            ...
            f"[bold yellow]{total_weight:.2f}[/bold yellow]",
        )

        console.print(table)
        if abs(total_weight - 100.0) > 0.01:
            console.print(
                f":warning: [bold yellow]Weights sum to {total_weight:.2f}% (not 100%)![/bold yellow]"
            )
    else:
...
def print_strategy(strategy: Strategy) -> None:
    ...
    console.print(
        f"[bold white]Stock Exchange:[/bold white] {strategy.stock_exchange.value}"
    )
    ...
    console.print(
        f"[bold white]Expected Returns:[/bold white] {strategy.expected_returns}"
    )
...
def print_analysis_response(analysis_response: AnalysisResponse) -> None:
    ...
    if hasattr(analysis_response, "is_approved"):
        ...
        console.print(
            f"[bold white]Portfolio Status:[/bold white] [{color}]{status}[/{color}]\n"
        )

    if getattr(analysis_response, "strengths", None):
        console.print(
            Panel.fit(
                f" [bold green]STRENGTHS[/bold green]\n{analysis_response.strengths}",
                style="green",
            )
        )
    ...
    if getattr(analysis_response, "weeknesses", None):
        console.print(
            Panel.fit(
                f" [bold red]WEAKNESSES[/bold red]\n{analysis_response.weeknesses}",
                style="red",
            )
        )
    ...
    if getattr(analysis_response, "overall_assessment", None):
        console.print(
            Panel.fit(
                f" [bold blue]OVERALL ASSESSMENT[/bold blue]\n{analysis_response.overall_assessment}",
                style="blue",
            )
        )
    ...
    if getattr(analysis_response, "advices", None):
        console.print(
            Panel.fit(
                f" [bold cyan]ACTIONABLE ADVICE[/bold cyan]\n{analysis_response.advices}",
                style="cyan",
            )
        )
