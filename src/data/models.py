...
class Benchmarks(Enum):
    ...
    ACWI = (
        "ACWI",
        "MSCI ACWI — All Country World Index covering developed and emerging markets",
    )
...
class PortfolioPreference(BaseModel):
    ...
    stock_exchange: StockExchange = Field(
        default=StockExchange.NYSE, description="Preferred stock exchange"
    )
    initial_investment: float = Field(
        ..., gt=0, description="Initial investment amount"
    )

    # Enhanced fields - Personal Information
    age: int = Field(..., ge=18, le=100, description="User's age")
    investment_knowledge: InvestmentKnowledge = Field(
        ..., description="Level of investment knowledge"
    )
    income_level: IncomeLevel = Field(..., description="Annual income level")

    # Enhanced fields - Investment Goals & Timeline
    investment_purpose: InvestmentPurpose = Field(
        ..., description="Purpose of investment"
    )
    liquidity_need: LiquidityNeed = Field(..., description="When funds might be needed")

    # Enhanced fields - Financial Situation
    has_emergency_fund: bool = Field(
        ..., description="Whether user has 3-6 months emergency fund"
    )
    other_investments: float = Field(
        default=0.0, ge=0, description="Value of other investments"
    )
    monthly_contribution: float = Field(
        default=0.0, ge=0, description="Planned monthly contribution amount"
    )

    # Enhanced fields - Risk Assessment
    max_acceptable_loss: float = Field(
        ..., ge=0, le=100, description="Maximum acceptable loss percentage in a year"
    )
    market_downturn_reaction: MarketDownturnReaction = Field(
        ..., description="Reaction to market downturn"
    )
    investment_priority: InvestmentPriority = Field(
        ..., description="Investment priority"
    )
...
class AssetAllocation(BaseModel):
    stocks_percentage: float | None = None
    bonds_percentage: float | None = None
    real_estate_percentage: float | None = None
    commodities_percentage: float | None = None
    cryptocurrency_percentage: float | None = None
...
class Strategy(BaseModel):
    ...
    description: Annotated[str | None, "Description of the investment strategy"]
    asset_allocation: Annotated[
        AssetAllocation, "Asset allocation for the investment strategy"
    ]
    geographical_diversification: Annotated[
        GeographicalDiversification,
        "Geographical diversification for the investment strategy",
    ]
    sector_diversification: Annotated[
        SectorDiversification, "Sector diversification for the investment strategy"
    ]
    stock_exchange: Annotated[
        StockExchange, "Stock exchange where the ETFs have to be quoted"
    ] = Field(default=StockExchange.NYSE)
