...
class TestAllDataValidation:
    ...
    def test_all_data_empty_benchmark_ticker(self, sample_portfolio):
        ...
        with pytest.raises(
            ValueError, match="benchmark_ticker must be a non-empty string"
        ):
    ...
    def test_all_data_holding_missing_symbol(self, sample_strategy):
        ...
        portfolio = Portfolio(
            name="Bad Portfolio", holdings=[holding], strategy=sample_strategy
        )
    ...
    def test_all_data_zero_total_weight(self, sample_strategy):
        ...
        portfolio = Portfolio(
            name="Zero Weight Portfolio", holdings=[holding], strategy=sample_strategy
        )

        with pytest.raises(
            ValueError, match="Portfolio weights must sum to a positive value"
        ):
            all_data(portfolio, "ACWI")
...
class TestAllDataFetching:
    ...
    def test_all_data_fetch_error(
        self, mock_today, mock_two_years_ago, mock_fetch, sample_portfolio
    ):
    ...
    @patch("src.utils.analysis_data.get_two_year_ago_date")
    @patch("src.utils.analysis_data.get_today_date")
    def test_all_data_no_data_retrieved(
        self, mock_today, mock_two_years_ago, mock_fetch, sample_portfolio
    ):
    ...
    @patch("src.utils.analysis_data.get_two_year_ago_date")
    @patch("src.utils.analysis_data.get_today_date")
    def test_all_data_missing_benchmark(
        self, mock_today, mock_two_years_ago, mock_fetch, sample_portfolio
    ):
        ...
        mock_fetch.return_value = {
            ...
            "BND": [
                {"date": "2023-01-01", "close": 80},
                {"date": "2023-01-02", "close": 81},
            ],
        }

        with pytest.raises(
            ValueError, match="Failed to fetch data for benchmark ticker"
        ):
            all_data(sample_portfolio, "ACWI")
    ...
    @patch("src.utils.analysis_data.get_two_year_ago_date")
    @patch("src.utils.analysis_data.get_today_date")
    def test_all_data_missing_portfolio_tickers(
        self, mock_today, mock_two_years_ago, mock_fetch, sample_portfolio
    ):
        ...
        mock_fetch.return_value = {
            ...
            "ACWI": [
                ...
                {"date": "2023-01-02", "close": 96},
            ],
            # Missing BND and VNQ
        }

        with pytest.raises(
            ValueError, match="Failed to fetch data for portfolio tickers"
        ):
            all_data(sample_portfolio, "ACWI")
    ...
    @patch("src.utils.analysis_data.get_two_year_ago_date")
    @patch("src.utils.analysis_data.get_today_date")
    def test_all_data_insufficient_data_points(
        self, mock_today, mock_two_years_ago, mock_fetch, sample_portfolio
    ):
