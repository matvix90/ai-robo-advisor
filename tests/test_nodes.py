...
class TestInvestmentStrategyNode:
    ...
    def test_investment_strategy_creates_strategy(
        self, sample_state, mock_llm_with_strategy_response, sample_investment_agent
    ):
        """Test that investment_strategy creates a strategy."""
        sample_state["metadata"]["investment_llm_agent"] = (
            mock_llm_with_strategy_response
        )
    ...
    def test_investment_strategy_uses_preferences(
        self, sample_state, mock_llm_with_strategy_response
    ):
    ...
    def test_investment_strategy_reasoning_shown(
        self, sample_state, mock_llm_with_strategy_response, capsys
    ):
    ...
    def test_investment_strategy_reasoning_hidden(
        self, sample_state, mock_llm_with_strategy_response
    ):
...
class TestCreatePortfolioNode:
    ...
    def test_create_portfolio_node(
        self, sample_state, sample_strategy, sample_portfolio, mock_llm
    ):
        ...
        portfolio_agent = PortfolioAgent(
            portfolio=sample_portfolio, reasoning="Test reasoning"
        )
    ...
    def test_create_portfolio_uses_strategy(
        self, sample_state, sample_strategy, sample_portfolio, mock_llm
    ):
    ...
    def test_create_portfolio_reasoning_shown(
        self, sample_state, sample_strategy, sample_portfolio, mock_llm, capsys
    ):
        """Test that reasoning is shown when requested."""
        sample_state["data"]["investment"] = {
            "strategy": sample_strategy,
...
class TestAlignmentAnalysisNode:
    ...
    def test_analyze_alignment_reasoning_shown(
        self, sample_state_with_portfolio, mock_llm, capsys
    ):
        """Test that reasoning is shown when requested."""
...
class TestPerformanceAnalysisNode:
    ...
    @patch("src.nodes.analyst_agents.performance.all_data")
    @patch("src.nodes.analyst_agents.performance.analyze_portfolio")
    def test_analyze_performance_sets_benchmark(
        self,
        mock_analyze_portfolio,
        mock_all_data,
        sample_state_with_portfolio,
    ...
    @patch("src.nodes.analyst_agents.performance.all_data")
    @patch("src.nodes.analyst_agents.performance.analyze_portfolio")
    def test_analyze_performance_poor_performance(
        self,
        mock_analyze_portfolio,
        mock_all_data,
        sample_state_with_portfolio,
...
class TestFeesAnalysisNode:
    ...
    def test_analyze_ter_expensive_funds(self, sample_state_with_portfolio, mock_llm):
        ...
        analysis_agent = AnalysisAgent(
            ...
            advices=[
                "Consider switching to Vanguard Total Market (VTI) with 0.03% TER",
    ...
    def test_analyze_ter_reasoning_shown(
        self, sample_state_with_portfolio, mock_llm, capsys
    ):
        """Test that reasoning is shown when requested."""
        from src.nodes.analyst_agents.fees import analyze_ter
    ...
    def test_analyze_ter_reasoning_hidden(
        self, sample_state_with_portfolio, mock_llm, capsys
    ):
        """Test that reasoning is hidden when not requested."""
        from src.nodes.analyst_agents.fees import analyze_ter
    ...
    def test_analyze_ter_initializes_analysis_dict(
        self, sample_state_with_portfolio, mock_llm
    ):
    ...
    def test_analyze_ter_with_existing_analysis(
        self, sample_state_with_portfolio, mock_llm
    ):
    ...
    def test_analyze_ter_with_multiple_advices(
        self, sample_state_with_portfolio, mock_llm
    ):
...
class TestDiversificationAnalysisNode:
    ...
    def test_analyze_diversification_basic(self, sample_state_with_portfolio, mock_llm):
        ...
        assert (
            result["data"]["analysis"]["diversification"].status.key == "is_diversified"
        )
        assert result["data"]["analysis"]["diversification"].status.value is True

    def test_analyze_diversification_not_diversified(
        self, sample_state_with_portfolio, mock_llm
    ):
    ...
    def test_analyze_diversification_reasoning_shown(
        self, sample_state_with_portfolio, mock_llm, capsys
    ):
        """Test that reasoning is shown when requested."""
        sample_state_with_portfolio["metadata"]["show_reasoning"] = True
    ...
    def test_analyze_diversification_reasoning_not_shown(
        self, sample_state_with_portfolio, mock_llm, capsys
    ):
    ...
    def test_analyze_diversification_asset_class_issues(
        self, sample_state_with_portfolio, mock_llm
    ):
        ...
        assert result["data"]["analysis"]["diversification"].status.value is False
        assert (
            "equity" in result["data"]["analysis"]["diversification"].reasoning.lower()
        )
        assert "bond" in result["data"]["analysis"]["diversification"].reasoning.lower()

    def test_analyze_diversification_geographical_issues(
        self, sample_state_with_portfolio, mock_llm
    ):
    ...
    def test_analyze_diversification_sector_concentration(
        self, sample_state_with_portfolio, mock_llm
    ):
        ...
        assert (
            "technology"
            in result["data"]["analysis"]["diversification"].reasoning.lower()
        )
        assert len(result["data"]["analysis"]["diversification"].advices) == 3

    def test_analyze_diversification_multiple_issues(
        self, sample_state_with_portfolio, mock_llm
    ):
    ...
    def test_analyze_diversification_initializes_analysis_dict(
        self, sample_state_with_portfolio, mock_llm
    ):
        """Test that analysis dict is initialized if it doesn't exist."""
    ...
    def test_analyze_diversification_llm_prompt_structure(
        self, sample_state_with_portfolio, mock_llm
    ):
    ...
    def test_analyze_diversification_uses_portfolio_data(
        self, sample_state_with_portfolio, mock_llm
    ):
    ...
    def test_analyze_diversification_empty_advices_when_diversified(
        self, sample_state_with_portfolio, mock_llm
    ):
    ...
    def test_analyze_diversification_with_structured_output(
        self, sample_state_with_portfolio
    ):
...
class TestNodesIntegration:
    ...
    def test_investment_to_portfolio_flow(
        self, sample_state, mock_llm_with_strategy_response, sample_portfolio, mock_llm
    ):
